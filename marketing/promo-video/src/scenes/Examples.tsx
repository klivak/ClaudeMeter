...
import { MockDashboard, MockNotification, MockCodexPanel } from '../components/Mock';
    ...
    <Frame
      ...
      visual={
        <div style={{ display: 'flex', gap: u * 2.2 }}>
          {(['dark', 'light', 'midnight', 'sunset'] as const).map((s) => (
            <MockDashboard key={s} theme={s} width={w} drift={false} />
          ))}
        </div>
      }
    />
...
const Alerts: React.FC<CardProps> = (p) => {
  ...
  return (
    <Frame
      ...
      accent={COLOR.red}
      visual={<MockNotification width={vertical ? vw(78) : vw(34)} />}
    />
  );
};
...
const Codex: React.FC<CardProps> = (p) => {
  ...
  return (
    <Frame
      ...
      accent={COLOR.codex}
      visual={<MockCodexPanel width={vertical ? vw(80) : vw(36)} />}
    />
  );
};
